...
void adobe_cam_xyz(const string & t_make, const string & t_model, float * cam_xyz) {
    ...
    string name = t_make + " " + t_model;
    int numEntries = sizeof (table) / sizeof(* table);
    for (int i = 0; i < numEntries; ++i) {
        if (!strncasecmp(name.c_str(), table[i].prefix, strlen(table[i].prefix))) {
            for (int j = 0; j < 12; j++ ) {
                cam_xyz[j] = table[i].trans[j] / 10000.0;
            }
            break;
        }
    }
